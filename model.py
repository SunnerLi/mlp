...
class MlpGMM(object):
    ...
    def getGenerator(self, noise_ph, name=None):
        ...
        with tf.variable_scope('generator'):
            ...
            self.g_fc2 = leaky_relu(DenseLayer(self.g_fc1, unit=self.fc_size * 2, name='g_fc2'))
            self.g_fc3 = leaky_relu(DenseLayer(self.g_fc2, unit=self.fc_size * 4, name='g_fc3'))
    ...
    def getDiscriminator(self, noise_ph, image_ph, name=None):
        ...
        with tf.variable_scope(name):
            # Construct true discriminator
            self.d_true_fc1 =leaky_relu(DenseLayer(image_ph, unit=self.fc_size * 4, name='d_fc1'))
            self.d_true_fc2 =leaky_relu(DenseLayer(self.d_true_fc1, unit=self.fc_size * 2, name='d_fc2'))
            ...
            # Construct fake discriminator
            self.d_fake_fc1 = leaky_relu(DenseLayer(self.g_tensor, unit=self.fc_size * 4, name='d_fc1', reuse=True))
            self.d_fake_fc2 = leaky_relu(DenseLayer(self.d_fake_fc1, unit=self.fc_size * 2, name='d_fc2', reuse=True))
            ...
            self.d_inv_fc3 = reverse_leaky_relu(reverse_DenseLayer(self.d_inv_fc4, unit=self.fc_size * 2, name='d_fc3'))
            self.d_inv_fc2 = reverse_leaky_relu(reverse_DenseLayer(self.d_inv_fc3, unit=self.fc_size * 4, name='d_fc2'))
